...
- **约束符号全局防重叠**：撒播阶段即保证所有符号（格间/笼间三角、格间/笼间等号）互不重叠、不压和值标签——撒播端与渲染端共用同一套位置计算（`pickCageEndpoints`/中点公式），新符号与已占用符号保持最小间距，冲突候选直接放弃
- **和值标签**：白底黑字小标签，无描边，底框边与笼边框线精确对齐（整齐嵌入笼左上角）；候选数位置向中心集中、字号调大、颜色加深
- **等值符号可读性**：等号加大（约半格宽、两横加粗），白底无描边；笼间等号与引导虚线同向（避免误读为对角线格间标记），与笼边框重叠时依然清晰
- **帮助弹窗**：版权、项目简介、玩法（深蓝=格间 / 金色=笼间 颜色区分说明）、符号速查（含一句话记忆口诀）、功能介绍
