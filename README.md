...
- **三档难度**：简单（35 给定，笼间约束 1~2 对，5 提示）/ 普通（18 给定，笼间约束 2~3 对，3 提示）/ 困难（5~9 给定，笼间约束 2~4 对，1 提示）
- **唯一解 + 逻辑可解**：暴力回溯验证唯一性，逻辑求解器验证每步可推导，不靠猜测
- **离线题库**：easy 20 / normal 20 / hard 10 题，前端按难度抽取，运行时生成作为 fallback
...
题库输出到 `public/puzzles/<难度>.json`，前端启动时加载。追加模式不会覆盖历史题目，适合扩充题库。
